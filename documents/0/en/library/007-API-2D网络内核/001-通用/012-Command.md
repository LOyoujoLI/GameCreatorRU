...
>This class is the event command data class in the event page ([CommandPage](?file=007-API-2D网络内核/001-通用/013-CommandPage))<br>Support for custom event commands<br>Related categories:[Command](?file=007-API-2D网络内核/001-通用/012-Command)、[CommandPage](?file=007-API-2D网络内核/001-通用/013-CommandPage)、[CommandTrigger](?file=007-API-2D网络内核/001-通用/014-CommandTrigger)<br><br>
...
##### Parameters
	...
	gameFunc [Optional] Default=null If present, the methods inside the GameFunction will be executed, otherwise you need to create the corresponding

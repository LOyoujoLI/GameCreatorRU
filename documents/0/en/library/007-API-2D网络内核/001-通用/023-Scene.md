...
| **modelID** : number;<br>The corresponding map model ID in the copy Non-copy = 0  |
...
| ---  |
| **[parse](#parse)**(jsonObj : any,  gameData : [GameData](?file=007-API-2D网络内核/001-通用/017-GameData)): void<br>This function is called when the system initializes the scene and can be overridden by the project layer to implement the required game logic
...
| **[getRealWidth](#getRealWidth)**(scene : [Scene](?file=007-API-2D网络内核/001-通用/023-Scene)): { width number; height number; }<br>[Static] Get the actual width and height calculated with the scene grid (normalized width and height by grid)
...
Needs to be implemented by the project layer, such as which scene object resources need to be preloaded when transferred from the server
...
Project layer implementation is required, such as what resources need to be preloaded for transfer from the server<br>
<br>
...
-- Custom events: you need to add logic to the project layer, for example, you can override preLoadCommandPage.preLoadCommandPage to add the content that needs to be preloaded
### dataLayers
###### dataLayers : number[][][];
...
## parse
###### **[parse](#parse)**(jsonObj : any,  gameData : [GameData](?file=007-API-2D网络内核/001-通用/017-GameData)): void :
...
## getRealWidth
###### **[getRealWidth](#getRealWidth)**(scene : [Scene](?file=007-API-2D网络内核/001-通用/023-Scene)): { width number; height number; } :

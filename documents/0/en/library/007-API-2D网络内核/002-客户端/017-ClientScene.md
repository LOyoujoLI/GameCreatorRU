...
| **[addSceneObject](#addSceneObject)**(soData : [SceneObject](?file=007-API-2D网络内核/001-通用/024-SceneObject),  isEntity? : boolean,  useModelClass? : boolean): ClientSceneObject<br>Scene objects are added to the scene
| **[removeSceneObject](#removeSceneObject)**(so : [SceneObject](?file=007-API-2D网络内核/001-通用/024-SceneObject),  removeFromList? : boolean): ClientSceneObject<br>Removing scene objects from the scene
| **[addPlayer](#addPlayer)**(player : [Player](?file=007-API-2D网络内核/001-通用/022-Player)): void<br>Add Player
| **[removePlayer](#removePlayer)**(player : [Player](?file=007-API-2D网络内核/001-通用/022-Player)): void<br>Remove player
| **[getGlobalPos](#getGlobalPos)**(localX : number,  localY : number): [Point](?file=007-API-2D网络内核/001-通用/019-Point)<br>Get the absolute mouse position (relative to the stage) according to the specified scene position
...
| **[on](#on)**(type : string,  caller : any,  listener : Function,  args? : Array<any>): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene)<br>Use the scenario to register an event listener object of the specified type to enable the listener to receive event notifications
| **[once](#once)**(type : string,  caller : any,  listener : Function,  args? : Array<any>): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene)<br>Use the scenario to register an event listener object of the specified type to enable the listener to receive event notifications, which are automatically removed after one response to the listener event
| **[off](#off)**(type : string,  caller : any,  listener : Function,  onceOnly? : boolean): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene)<br>Remove the listener from the scene
| **[offAll](#offAll)**(type? : string): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene)<br>Removes all listeners of the specified event type from the scene
## Protected Method
|<div style="width:1000px;text-align:left" >Method</div>   |
| ---  |
...
## addSceneObject
###### **[addSceneObject](#addSceneObject)**(soData : [SceneObject](?file=007-API-2D网络内核/001-通用/024-SceneObject),  isEntity? : boolean,  useModelClass? : boolean): ClientSceneObject :
...
##### Parameters
	...
	useModelClass [Optional] Default = true Whether to use the implementation class of the scene object model, which is usually true at the project level

##### Return
[ClientSceneObject] Example of added scene objects

## removeSceneObject
###### **[removeSceneObject](#removeSceneObject)**(so : [SceneObject](?file=007-API-2D网络内核/001-通用/024-SceneObject),  removeFromList? : boolean): ClientSceneObject :
...
## addPlayer
###### **[addPlayer](#addPlayer)**(player : [Player](?file=007-API-2D网络内核/001-通用/022-Player)): void :
Add Player
##### Parameters
	player
...
## removePlayer
###### **[removePlayer](#removePlayer)**(player : [Player](?file=007-API-2D网络内核/001-通用/022-Player)): void :
Remove player
##### Parameters
	player
...
## getGlobalPos
###### **[getGlobalPos](#getGlobalPos)**(localX : number,  localY : number): [Point](?file=007-API-2D网络内核/001-通用/019-Point) :
...
## on
###### **[on](#on)**(type : string,  caller : any,  listener : Function,  args? : Array<any>): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene) :
...
## once
###### **[once](#once)**(type : string,  caller : any,  listener : Function,  args? : Array<any>): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene) :
...
## off
###### **[off](#off)**(type : string,  caller : any,  listener : Function,  onceOnly? : boolean): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene) :
...
## offAll
###### **[offAll](#offAll)**(type? : string): [ClientScene](?file=007-API-2D网络内核/002-客户端/017-ClientScene) :

...
| **currentScene** : ClientScene;<br>The current scene, the project layer needs to set this value when implementing scene replacement  |
| **playerKey** : string;<br>My player unique key format uid_xxxxxx where UID is the unique ID, the same then represents the same player (repeated login will squeeze the number)  |

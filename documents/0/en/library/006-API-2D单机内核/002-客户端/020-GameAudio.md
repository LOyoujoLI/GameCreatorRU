# GameAudio Audio
>Support for smooth transition volume<br>Support for tone (playback rate)<br
...
| **[playSE](#playSE)**(url : string,  volume? : number,  pitch? : number,  soc? : [ClientSceneObject](?file=006-API-2D单机内核/002-客户端/018-ClientSceneObject)): SoundChannel<br>[Static] Play sound effects, play on the scene object is based on the current scene of the camera and the distance to the target to change the sound size
| **[stopSE](#stopSE)**(channels? : any): void<br>[Static] Stop SE
| **[playTS](#playTS)**(url : string,  volume? : number,  pitch? : number,  soc? : [ClientSceneObject](?file=006-API-2D单机内核/002-客户端/018-ClientSceneObject)): SoundChannel<br>[Static] Play the voice, when playing on the scene object, then change the sound size according to the distance between the camera and the target of the current scene
...
[Static] Play background music: Only replay if the addresses are not equal, but the volume will change (if you need to replay, you can stop the BGM and then play it again)<br> 
Only one background music can be played at the same time globally, if you need to overlay play audio, please use playSE<br> 
The same address but the tone is not the same will also replay
##### Parameters
...
[Static] Play ambient sound effects : replay only if the addresses are not equal, but the volume will change (if you need to replay, you can stop the BGM and then play it again)<br> 
Only one ambient sound effect can be played at the same time globally, if you need to overlay play audio, please use playSE<br> 
The same address but the pitch is not the same will also replay)
##### Parameters
...
## playSE
###### **[playSE](#playSE)**(url : string,  volume? : number,  pitch? : number,  soc? : [ClientSceneObject](?file=006-API-2D单机内核/002-客户端/018-ClientSceneObject)): SoundChannel :
...
## playTS
###### **[playTS](#playTS)**(url : string,  volume? : number,  pitch? : number,  soc? : [ClientSceneObject](?file=006-API-2D单机内核/002-客户端/018-ClientSceneObject)): SoundChannel :
